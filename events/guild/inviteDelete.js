...
module.exports = async (client, invite) => {
  ...
  const embed = new Discord.EmbedBuilder()
    .setTitle(lang.messages.inviteDelete.title)
    .setColor(config.Bot.EmbedColor)
    .setAuthor({ name: invite.guild.name, iconURL: invite.guild.iconURL({ dynamic: true }) })
    .setDescription(lang.messages.inviteDelete.description.replace('{username}', `<@${fetchModerator.executor.id}>`).replace('{channelName}', `${invite.channel}`))
    .addFields(lang.messages.inviteDelete.fields.map(field => ({
        name: field.name,
        value: field.value
            .replace('{inviteCode}', invite.code)
            .replace('{inviteURL}', invite.url)
            .replace('{inviteMaxUses}', invite.maxUses)
